...
@param symParams    [Input]       A symmetric algorithm parameters object. AES 
                                  is the only algorithm currently supported.
@param symKey       [Input]       A symmetric algorithm key object. AES is the 
                                  only algorithm currently supported.
@param mode         [Input]       The mode of operation. The acceptable values 
                                  are <tt>SB_CCM</tt>, <tt>SB_CCM_STAR</tt> and 
                                  <tt>SB_GCM</tt>.
@param authEncKey [Input/Output] The Authenticated Encryption key object.
...
@param symParams    [Input]  The symmetric algorithm parameters object. AES is 
                             the only algorithm currently supported.
@param authEncKey   [Input]  An authenticated encryption key
@param nonceLen     [Input]  The length (in bytes) of the nonce. For 
                             <tt>SB_CCM</tt> and <tt>SB_CCM_STAR</tt>, 
			     acceptable values are <tt>7</tt>, <tt>8</tt>, 
			     <tt>9</tt>, <tt>10</tt>, <tt>11</tt>, <tt>12</tt> 
			     and <tt>13</tt>. For <tt>SB_GCM</tt>, 
			     <tt>nonceLen</tt> must be greater than <tt>0</tt> 
			     and the recommended value is <tt>12</tt>.
@param nonce        [Input]  Nonce buffer.
@param addDataLen   [Input]  The length (in bytes) of additional data that will 
                             be processed. This is optional for <tt>SB_GCM</tt>.
@param plaintextLen [Input]  The length (in bytes) of plaintext that will be 
                             processed. This is optional for <tt>SB_GCM</tt>.
@param macLen       [Input]  The length of the MAC used for authentication. For 
                             <tt>SB_CCM</tt> and <tt>SB_CCM_STAR</tt>, 
			     acceptable values are <tt>4</tt>, <tt>6</tt>, 
			     <tt>8</tt>, 
			     <tt>10</tt>, <tt>12</tt>, <tt>14</tt> and 
			     <tt>16</tt>. <tt>SB_CCM_STAR</tt> may also specify 
			     a <tt>macLen</tt> of <tt>0</tt> if authentication 
			     is not required. For <tt>SB_GCM</tt>, 
			     <tt>macLen</tt> must be greater than or equal to 
			     <tt>8</tt> and less than or equal to <tt>16</tt>. 
			     If the reccomended value of <tt>12</tt> is 
			     <em>not</em> used for the <tt>nonceLen</tt>, the 
			     <tt>macLen</tt> must be <tt>16</tt>.
@param authEncCtx   [Output] The Authenticated Encryption context object 
                             pointer.
@param sbCtx        [Input]  Global context.
...
@param authEncCtx   [Input]  Authenticate encryption context object.
@param addDataLen   [Input]  The length (in bytes) of additional data. If the 
                             mode is <tt>SB_CCM_STAR</tt> and the 
			     <tt>macLen</tt> specified in 
			     <tt>hu_authEncBegin()</tt> was 0, then 
			     <tt>addDataLen</tt> must be <tt>0</tt>.
@param addData      [Input]  The additional data buffer.
@param sbCtx        [Input]  Global context.
...
@param authEncCtx     [Input]  Authenticate encryption context object.
@param plaintextLen   [Input]  The length (in bytes) of plaintext.
...
@param authEncCtx      [Input]  Authenticate encryption context object.
@param ciphertextLen   [Input]  The length (in bytes) of ciphertext.
...
@param authEncCtx     [Input/Output] The AuthEnc context object pointer.
@param macLen         [Input]        The length of the mac buffer in bytes. The 
                                     <tt>macLen</tt> must match the value 
				     supplied in the <tt>hu_AuthEncBegin()</tt> 
				     function.
@param mac            [Output]       The mac buffer.
@param sbCtx          [Input]        Global context.
...
@param authEncCtx [Input/Output] The AuthEnc context object pointer.
@param macLen     [Input]        The length of the mac buffer in bytes. The 
                                 <tt>macLen</tt> must match the value supplied 
				 in the <tt>hu_AuthEncBegin()</tt> function.
@param mac        [Input]        The mac buffer.
@param sbCtx      [Input]        Global context.
...
@param symParams    [Input]  A symmetric algorithm parameters object. AES is 
                             the only algorithm currently supported.
@param mode         [Input]  The mode of operation. The acceptable values are
                             <tt>SB_CCM</tt>, <tt>SB_CCM_STAR</tt> and 
			     <tt>SB_GCM</tt>.
@param authEncKey   [Input]  An authenticated encryption key
@param nonceLen     [Input]  The length (in bytes) of nonce. For <tt>SB_CCM</tt> 
...
@param plaintextLen [Input]  The length (in bytes) of plaintext.
@param plaintext    [Input]  The plaintext buffer.
@param macLen       [Input]  The length (in bytes) of the mac buffer. The 
                             <tt>macLen</tt> must match the value supplied in 
			     the <tt>hu_AuthEncBegin()</tt> function.
@param mac          [Output] The mac buffer.
@param ciphertext   [Output] The ciphertext buffer.
@param sbCtx        [Input]  A global context.
...
@param mode          [Input]  The mode of operation. The acceptable values are 
			      ...
			      <tt>SB_GCM</tt>.
@param authEncKey    [Input]  An authenticated encryption key
@param nonceLen      [Input]  The length (in bytes) of nonce. For 
                              <tt>SB_CCM</tt> and <tt>SB_CCM_STAR</tt>, 
			      acceptable values are <tt>7</tt>, <tt>8</tt>, 
			      <tt>9</tt>, <tt>10</tt>, <tt>11</tt>, <tt>12</tt> 
			      and <tt>13</tt>. For <tt>SB_GCM</tt>, 
			      <tt>nonceLen</tt> must be greater than <tt>0</tt>,
                              and the recommended value is <tt>12</tt>.
@param nonce         [Input]  Nonce buffer.
@param addDataLen    [Input]  The length (in bytes) of additional data that 
                              will be processed. This is optional for 
			      <tt>SB_GCM</tt>.
@param addData       [Input]  The additional data buffer.
@param ciphertextLen [Input]  The length (in bytes) of ciphertext.
@param ciphertext    [Input]  The ciphertext buffer.
