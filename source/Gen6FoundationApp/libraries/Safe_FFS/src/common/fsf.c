...
{
#ifdef HCC_UNICODE
	return (char*)("EFFS_STD_UNI ver:2.43");
#else
	return (char*)("File System standard version:2.43");
#endif
}
...
int _fg_setdename(const W_CHAR *s, FS_VOLUMEINFO *vi, FS_DIRENTRY *de) 
{
	...
	if (len>=(int)(FS_MAXLNAME)) return 1; /* too long name */
...
int _fg_findpath(const FS_VOLUMEINFO *vi,FS_NAME *fsname) 
{
	...
	for (;*path;) 
	{
		...
		if (!_fg_findfile(vi,name,fsname->dirnum,&dirnum)) return 0;
		if (!(vi->direntries[dirnum].attr & FS_ATTR_DIR ) ) return 0;
...
int _fg_addentry(FS_VOLUMEINFO *vi,const FS_NAME *fsname,FS_DIRENTRY **pde) 
{
	...
	for (a=0; a<vi->maxdirentry; a++) 
	{
		...
		{ /* not used */
			...
			if (_fg_setdename(fsname->lname,vi,de)) return F_ERR_NOMOREENTRY;
...
int fg_wmkdir(FS_MULTI *fm,const W_CHAR *dirname) 
{
	...
	if (fsname.lname[0]=='.') return F_SETLASTERROR(F_ERR_INVALIDNAME);
...
FS_FILE *fg_wtruncate(FS_MULTI *fm,const W_CHAR *filename,unsigned long length) 
{
	...
	int a;
#ifndef HCC_UNICODE
	if (length==0) return fg_open(fm,filename,(char*)"w+");
#else
	if (length==0) 
	{
		W_CHAR m[3];
