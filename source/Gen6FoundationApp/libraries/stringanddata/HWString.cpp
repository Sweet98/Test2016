...
// HWString.cpp
//======================================================================================
// $RCSfile: HWString.cpp $
// $Revision: 1.19 $
// $Date: 2009/04/28 14:40:55EDT $
//======================================================================================
...
HWSTRING::HWSTRING(unsigned int DeclaredMaximumSize, bool InputAutoResize)
{
	...
	WChar = new wchar_t[ MaximumSize + 2 ];     // Leave space for a NULL.
    WChar[0] = NULL;
	MaximumSize = DeclaredMaximumSize;
...
// If the new maximum size is smaller than the current maximum size, but we're not supposed to allow that, quit now.
    ...
    Temp = new wchar_t[NewDeclaredMaximumSize + 2];    // Leave space for a NULL.
...
// Else we have one or more characters to compare.
	...
	// Compare the strings according to the input length.
	    ...
	    {
//			StringComparisonResult = wcsncmp( InputString, &WChar[LocalStringIndex], InputStringLength);
    		StringComparisonResult = 0;
...
// Else we have one or more characters to compare.
	...
	// is longer than the other.
		if(CompareIsCaseSensitive)
	    {
//			StringComparisonResult = wcsncmp( InputString, &WChar[LocalStringIndex], ShortestInputStringLength);
    		StringComparisonResult = 0;
...
// First, resize the output string.
	if( Status )
	{
		...
		NumberOfWideCharactersToCopy = Length / 2;
	...
	if( Status )
	{
		if( Direction == 1 )
		{
			...
			for( i = 0;    i < NumberOfWideCharactersToCopy;    ++i )
			{
				...
				NextHStringLocation += Direction * 2;
			}
		}
		else
		{
...
// is that we don't want to erase the existing string by setting Size to zero until we know that there will be enough room
// for the new string.
	Status = Reallocate( RangeLength( FirstCharPosition, LastCharPosition, LengthOfStringToCopyFrom ) / 2, false );
