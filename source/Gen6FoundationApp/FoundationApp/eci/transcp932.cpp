/*=================================================================================
  A helper for CTransCode to support CP932
//=================================================================================
   $Source: Gen6FoundationApp/FoundationApp/eci/transcp932.cpp $
...
//////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////
/// Handler for the CP932 translation.
/*!
*/
class CTransInfoCP932 : public CTransInfoBase
{
public:
	CTransInfoCP932(codepage_t codepage);
	virtual ~CTransInfoCP932();
protected:
...
CTransInfoCP932::CTransInfoCP932(codepage_t codepage)
...
, m_pBackTable(NULL)
{
	...
	{
		size_t Unicode = Table_CP932[i];
		m_pBackTable[Unicode] = (wchar_t) i;
	}
}

CTransInfoCP932::~CTransInfoCP932()
{
	delete m_pBackTable;
}

// Below is an atempt to create a memory saving Unicode to CP932 translation (slow).
// While writing it I decided to use the table version. I keep the code here for reference.
//size_t CTransInfoCP932::SearchInTable(wchar_t search, wchar_t start, wchar_t end)
//{
//	size_t Index=0;
//	for(size_t i=start; i<=end; i++)
//	{
//		if(search==Table_CP932[i])
//		{
//			Index=i;
//			break;
//		}
...
//	return Index;
//}
// CAUTION: output buffer must be big enough. CP932 can be 2x bigger than UTF16!
//MatrixError_t CTransInfoCP932::FromUnicode(const wchar_t *pIn, size_t length, bchar_t *pOut, size_t &DestinationSize)
//{
...
MatrixError_t CTransInfoCP932::FromUnicode(const wchar_t *pIn, bchar_t *pOut, size_t &DestinationSize)
{
	return FromUnicode(pIn, wcslen(pIn), pOut, DestinationSize);
}

MatrixError_t CTransInfoCP932::ToUnicode(const bchar_t *pIn, size_t length, wchar_t *pOut, size_t &DestinationSize)
{
	MatrixError_t RetVal=ERR_PASS;
	size_t Index=0;
...
MatrixError_t CTransInfoCP932::ToUnicode(const bchar_t *pIn, wchar_t *pOut, size_t &DestinationSize)
{
	return ToUnicode(pIn, strlen(pIn), pOut, DestinationSize);
}

///////////////////////////////////////////////////////////////////////////////
//! Create the CP932 handler.
/*!
*/
void	CTransCode::CreateCp932Translator(void)
{
	const int codepage = CP932;
	if(m_TransInfo[codepage] == NULL)	// only create if not done yet (memory leak!)
	{
		m_TransInfo[codepage] = new CTransInfoCP932(codepage);
	}
}
